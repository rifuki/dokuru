...
impl Section5 {
    ...
    fn no_running_containers_result(
        ...
        CheckResult {
            ...
            message:
                "No running containers to evaluate; start workloads and rerun this runtime check"
                    .into(),
            affected: vec![],
            remediation_kind,
            audit_command: None,
                ...
                Box::pin(async move {
                    if containers.is_empty() {
                        return Ok(CheckResult {
                            ...
                            status: CheckStatus::Error,
                            message: "No running containers to evaluate; start workloads and rerun this runtime check".into(),
                            affected: vec![],
                ...
                Box::pin(async move {
                    if containers.is_empty() {
                        return Ok(CheckResult {
                            ...
                            status: CheckStatus::Error,
                            message: "No running containers to evaluate; start workloads and rerun this cgroup check".into(),
                            affected: vec![],
                            remediation_kind: RemediationKind::Manual,
                            audit_command: None,
                            raw_output: None,
