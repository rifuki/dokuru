...
pub(crate) fn coverage_card(group: &'static CoverageGroup, index: usize) -> impl IntoView {
    view! {
        <div data-testid=format!("coverage-group-{}", index) class="hover-rise bg-[#09090B] p-5 sm:p-8 flex flex-col">
            ...
            <h3 class="font-heading text-xl md:text-2xl font-bold text-white break-words">{group.title}</h3>
