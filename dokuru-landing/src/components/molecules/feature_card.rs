...
pub(crate) fn feature_card(feature: &'static Feature, index: usize) -> impl IntoView {
    ...
    view! {
        <div node_ref=card_ref data-testid=format!("feature-card-{}", index) class="reveal hover-lift group relative bg-[#09090B] border border-white/10 rounded-xl p-5 transition-all duration-300 hover:border-[#2496ED]/40 hover:shadow-[0_30px_60px_-20px_rgba(36,150,237,0.25)] sm:p-8" style=format!("--motion-delay: {}ms; --motion-duration: 500ms", index * 100)>
            ...
            <h3 class="font-heading text-xl md:text-2xl font-bold text-white leading-tight break-words">{feature.title}</h3>
            <p class="mt-3 text-zinc-400 leading-relaxed text-[15px]">{feature.body}</p>
            <ul class="mt-6 flex flex-wrap gap-2">
                {feature.points.iter().map(|point| view! { <li class="max-w-full break-words rounded-full border border-white/10 bg-white/[0.03] px-3 py-1 font-mono text-[11px] text-zinc-300">{*point}</li> }).collect_view()}
            </ul>
        </div>
    }
}
