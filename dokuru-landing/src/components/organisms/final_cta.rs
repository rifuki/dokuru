                ...
                <div node_ref=card_ref class="reveal relative rounded-2xl border border-white/10 bg-[#09090B] overflow-hidden">
                    ...
                    <div class="relative p-5 sm:p-10 md:p-16 grid lg:grid-cols-12 gap-10 items-center">
                        <div class="lg:col-span-7">
                            ...
                            <h2 node_ref=heading_ref class="reveal font-heading text-3xl md:text-4xl lg:text-5xl font-bold tracking-tight text-white leading-[1.06]" style="--motion-delay: 100ms; --motion-duration: 500ms">"Install the agent."<br class="hidden sm:block"/><span class="sm:hidden">" "</span>"Open its dashboard."</h2>
                            ...
                            <div node_ref=actions_ref class="reveal mt-8 flex flex-wrap items-center gap-3" style="--motion-delay: 300ms; --motion-duration: 500ms">
                                <a href="#how-it-works" data-testid="final-cta-primary" class="group inline-flex w-full items-center justify-center gap-2 bg-[#2496ED] hover:bg-[#1C7CBA] text-white font-medium px-6 py-3.5 rounded-xl shadow-sm transition-colors active:scale-[0.98] sm:w-auto">"Install Agent" {icon(IconKind::ArrowRight, 16, "transition-transform group-hover:translate-x-0.5", "2")}</a>
                                <a href=APP_URL target="_blank" rel="noopener noreferrer" data-testid="final-cta-secondary" class="inline-flex w-full items-center justify-center gap-2 bg-white/[0.03] hover:bg-white/[0.07] border border-white/10 hover:border-white/25 text-white font-medium px-6 py-3.5 rounded-xl transition-colors active:scale-[0.98] sm:w-auto">{icon(IconKind::Terminal, 16, "text-[#2496ED]", "2")}"Open App"</a>
                            </div>
                        </div>
                        <div node_ref=preview_ref class="reveal lg:col-span-5" data-reveal="left" style="--motion-delay: 200ms; --motion-duration: 600ms">
                            <div class="rounded-xl border border-white/10 bg-black/60 backdrop-blur-sm overflow-hidden">
                                ...
                                <div class="p-4 font-mono text-[11px] leading-6 text-zinc-200 sm:p-5 sm:text-[13px] sm:leading-7">
                                    <div class="break-all"><span class="text-[#2496ED]">"$"</span> " curl -fsSL " <span class="text-[#00E5FF]">"https://dokuru.rifuki.dev/install"</span> " | bash"</div>
                                    <div class="text-zinc-500 text-[11px] mt-2">"# Open the agent dashboard from the install output"</div>
                                    <div class="text-zinc-500 text-[11px]">"# Add to app only for multi-host control" <span class="terminal-cursor"/></div>
                                </div>
                            </div>
                        </div>
                    </div>
