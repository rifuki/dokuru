                    ...
                    <div class="relative p-5 sm:p-10 md:p-16 grid lg:grid-cols-12 gap-10 items-center">
                        <div class="lg:col-span-7">
                            ...
                            <div node_ref=actions_ref class="reveal mt-8 flex flex-wrap items-center gap-3" style="--motion-delay: 300ms; --motion-duration: 500ms">
                                <a href="#how-it-works" data-testid="final-cta-primary" class="group inline-flex items-center gap-2 bg-[#2496ED] hover:bg-[#1C7CBA] text-white font-medium px-6 py-3.5 rounded-xl shadow-sm transition-colors active:scale-[0.98]">"Install Agent" {icon(IconKind::ArrowRight, 16, "transition-transform group-hover:translate-x-0.5", "2")}</a>
                                <a href=APP_URL target="_blank" rel="noopener noreferrer" data-testid="final-cta-secondary" class="inline-flex items-center gap-2 bg-white/[0.03] hover:bg-white/[0.07] border border-white/10 hover:border-white/25 text-white font-medium px-6 py-3.5 rounded-xl transition-colors active:scale-[0.98]">{icon(IconKind::Terminal, 16, "text-[#2496ED]", "2")}"Open App"</a>
                            </div>
                        </div>
