        ...
        <section id="how-it-works" data-testid="how-it-works-section" class="relative py-24 md:py-32 border-t border-white/5 overflow-hidden">
            ...
            <div class="max-w-7xl mx-auto px-5 md:px-10">
                <div node_ref=heading_ref class="reveal mb-16 flex flex-col items-center text-center">
                    ...
                    <h2 class="font-heading text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-bold tracking-tight text-white leading-[1.1] max-w-3xl">
                    ...
                    <div class="mt-10 w-full max-w-[430px]">
                        {mode_tabs(active_mode)}
                    </div>
                </div>

                <div class="grid min-w-0 lg:grid-cols-12 gap-8 lg:gap-12 items-start mt-8">
                    <ol class="lg:col-span-5 space-y-3">
                        {(0..3).map(|i| step_item(active_mode, active_step, i)).collect_view()}
                    </ol>

                    <div node_ref=panel_ref class="reveal relative mt-8 min-w-0 lg:col-span-7 lg:mt-0" data-reveal="left" style="--motion-delay: 200ms">
                        ...
                        <div class="mt-4 flex flex-col gap-1 px-2 font-mono text-[10px] uppercase tracking-[0.18em] text-zinc-500 sm:flex-row sm:justify-between sm:tracking-[0.2em]">
...
fn mode_tabs(active_mode: RwSignal<WorkflowMode>) -> impl IntoView {
    view! {
        <div class="relative grid w-full max-w-[430px] grid-cols-2 rounded-full border border-white/10 bg-white/[0.02] p-1.5 backdrop-blur-sm shadow-xl">
...
fn mode_tab(
    ...
    view! {
        <button type="button" on:click=move |_| active_mode.set(mode) class=move || if active_mode.get() == mode { "relative z-10 flex h-11 cursor-pointer items-center justify-center gap-1.5 rounded-full px-3 text-white transition-colors sm:gap-2.5 sm:px-4" } else { "relative z-10 flex h-11 cursor-pointer items-center justify-center gap-1.5 rounded-full px-3 text-zinc-400 transition-colors hover:text-zinc-200 sm:gap-2.5 sm:px-4" }>
            <span class="font-heading text-[14px] font-semibold sm:text-[15px]">{title}</span>
            <span class=move || if active_mode.get() == mode { "font-mono text-[9px] uppercase tracking-wider text-[#2496ED] sm:text-[10px]" } else { "font-mono text-[9px] uppercase tracking-wider text-zinc-500 sm:text-[10px]" }>{label}</span>
        </button>
    }
}
...
fn step_item(
    ...
    view! {
        <li node_ref=step_ref data-testid=format!("how-step-{}", step_num(index)) class="reveal group" data-reveal="right" style=format!("--motion-delay: {}ms", index * 100)>
            <button type="button" on:click=move |_| active_step.set(index) class=move || if active_step.get() == index { "relative flex w-full cursor-pointer gap-4 overflow-hidden rounded-2xl border border-white/10 bg-gradient-to-br from-white/[0.06] to-transparent p-4 text-left shadow-lg ring-1 ring-white/5 transition-all before:absolute before:inset-0 before:bg-gradient-to-r before:from-[#2496ED]/10 before:to-transparent before:opacity-100 sm:gap-5 md:p-6" } else { "relative flex w-full cursor-pointer gap-4 overflow-hidden rounded-2xl border border-transparent p-4 text-left transition-all before:absolute before:inset-0 before:bg-gradient-to-r before:from-[#2496ED]/10 before:to-transparent before:opacity-0 hover:bg-white/[0.03] hover:before:opacity-50 sm:gap-5 md:p-6" }>
