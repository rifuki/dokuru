...
pub async fn update_config_field(
    ...
    let target = req.target.unwrap_or_else(|| "local".to_string());
    let path = req.path;
    let value = req.value;

    let response = tokio::task::spawn_blocking(move || {
        let path_refs: Vec<&str> = path.iter().map(String::as_str).collect();
        toml_config::write_field_to_toml(&target, &path_refs, &value)?;

        // Return updated local config content so the frontend can refresh the editor.
        let local_path = toml_config::local_config_path();
        let content = toml_config::read_local_config_string()?;

        Ok::<_, eyre::Report>(LocalConfigResponse {
...
fn config_snapshot(config: &Config) -> EffectiveConfigResponse {
    let field_sources = config_field_sources();
...
fn config_field_sources() -> BTreeMap<String, Vec<ConfigSourceDetail>> {
    let default_doc =
