...
pub trait EnvironmentRepository: Send + Sync {
    async fn create(&self, db: &Database, user_id: Uuid, token_id: Uuid, name: Option<&str>, ip: Option<&str>) -> eyre::Result<Environment>;
    async fn find_by_id(&self, db: &Database, id: Uuid, user_id: Uuid) -> eyre::Result<Option<Environment>>;
    async fn find_by_token(&self, db: &Database, token_id: Uuid) -> eyre::Result<Option<Environment>>;
    ...
    async fn update_info(&self, db: &Database, id: Uuid, name: Option<&str>, docker_version: Option<&str>) -> eyre::Result<()>;
...
impl EnvironmentRepository for EnvironmentRepositoryImpl {
    ...
    async fn find_by_token(&self, db: &Database, token_id: Uuid) -> eyre::Result<Option<Environment>> {
        let env = sqlx::query_as::<_, Environment>(
            "SELECT * FROM environments WHERE token_id = $1"
        )
        .bind(token_id)
        .fetch_optional(db.pool())
        .await?;
    ...
    async fn update_last_seen(&self, db: &Database, id: Uuid) -> eyre::Result<()> {
        sqlx::query(
            "UPDATE environments SET last_seen = now() WHERE id = $1"
        )
        .bind(id)
        .execute(db.pool())
        .await?;
