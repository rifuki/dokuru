...
use axum::{body::Body, http::{Request, Response, StatusCode, header, HeaderMap}};
...
pub async fn patch_authed(app: axum::Router, uri: &str, token: &str, body: &Value) -> (StatusCode, Value) {
    let req = Request::builder()
        .method("PATCH")
        .uri(uri)
        .header(header::AUTHORIZATION, format!("Bearer {}", token))
...
pub async fn post_json_with_cookie(app: axum::Router, uri: &str, body: &Value, cookie: &str) -> (StatusCode, Value) {
    let req = Request::builder()
        .method("POST")
        .uri(uri)
        .header(header::CONTENT_TYPE, "application/json")
